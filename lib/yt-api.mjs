...
// ---- go-live support (2026-08-01): bind → wait for feed → transition live ----

/** Full broadcast record: lifeCycleStatus + boundStreamId + monitorStream. */
export async function getBroadcast(id) {
  const j = await api('GET', 'liveBroadcasts', null, { part: 'snippet,status,contentDetails', id });
  const b = j.items?.[0];
  if (!b) throw new Error(`broadcast ${id} not found`);
  return {
    id: b.id,
    title: b.snippet.title,
    lifeCycleStatus: b.status.lifeCycleStatus, // created|ready|testStarting|testing|liveStarting|live|complete
    boundStreamId: b.contentDetails?.boundStreamId || null,
    monitorEnabled: b.contentDetails?.monitorStream?.enableMonitorStream !== false,
  };
}

/** The channel's reusable stream keys → [{id,title,status}] (status: active|ready|inactive).
 *  NOTE: never request part=cdn here — one of this account's streams 500s the
 *  whole list when cdn is included (probed 2026-08-01). */
export async function listStreams() {
  const j = await api('GET', 'liveStreams', null, { part: 'snippet,status', mine: 'true', maxResults: '50' });
  return (j.items || []).map((s) => ({
    id: s.id,
    title: s.snippet.title,
    status: s.status?.streamStatus,           // active = receiving data
    health: s.status?.healthStatus?.status,   // good|ok|bad|noData
  }));
}

/** Bind a broadcast to a stream key. */
export async function bindBroadcast(broadcastId, streamId) {
  return api('POST', 'liveBroadcasts/bind', null, { id: broadcastId, part: 'id,contentDetails', streamId });
}

/** Transition a broadcast (testing | live | complete). */
export async function transitionBroadcast(broadcastId, to) {
  return api('POST', 'liveBroadcasts/transition', null, { id: broadcastId, broadcastStatus: to, part: 'status' });
}
